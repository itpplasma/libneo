...
class MAW:
    def __init__( self , Shotnumber = None, Experiment = 'AUGD', Diagnostic = 'MAW',Binning=None ):
        self.Status = False
        self.Status_PSL = False
        self.Status_torangle = False
        self.Status_torangle_PSL = False
        if Shotnumber != None :
            self.Load( Shotnumber ,Binning=Binning)
...
##load data
    def Load( self, Shotnumber ,Experiment= 'AUGD', Diagnostic= 'MAW', tBegin=-1.0, tEnd=16.0, Edition = 0L, nRows = 2, nCoils = 8, labelRows = ['u','l'],Binning=None,thres=100.):
...
### bin data
    def Binning( self, samplefreq = 0.1 ):                   
        if self.Status:                                                               
            ...
            tmptime,self.I_DC1 = dataBinning(self.time,self.I_DC1,samplefreq=samplefreq )
            tmptime,self.I_DC2 = dataBinning(self.time,self.I_DC2,samplefreq=samplefreq )
...
### function to get toroidal number,phase, amplitude of the upper and lower coild set
        ...
        if self.Status:
            ...
            if useLSQ:
                ...
                for ul in np.arange(2):
                    ...
                    ##sort argument after toroidal angle
            ...
            #IPython.embed()
            #get the phase of the dominant toroidal number
            ...
            fphi=[interp1d(MPtime,phase[:,0],assume_sorted=False,bounds_error=False,fill_value =float('nan')),interp1d(MPtime,phase[:,1],assume_sorted=False,bounds_error=False,fill_value =float('nan'))]
    ...
    def PSLresponse(self):
        if self.Status:
            ##see at the end of the file, from http://www.aug.ipp.mpg.de/augtwiki/bin/view/CWAC/PslResponse            
            ...
            for i in np.arange(self.Icoils[0,:,0].size):
                for j in np.arange(self.Icoils[0,0,:].size):
                    ...
                    if i==0:
                        self.PSLIcoils[:,i,j] = np.fft.irfft(FFTIn*upperResp)
                    else:
                        self.PSLIcoils[:,i,j] = np.fft.irfft(FFTIn*lowerResp)
...
#function to map timebase of diagnostic on geometrical phi
    def getSynPhase(self,timeIn, tref=2.5,phiDiag=0.0, idxCoil=0,onlyPos=False, usePSL=True):
        ...
        if self.Status_torangle :
            ...
            synPhase= ( fphi[idxCoil](tref) - (fphi[idxCoil](timeIn)) ) + np.remainder(phiDiag,2.*np.pi/np.mean(self.n) )  
            if onlyPos:
