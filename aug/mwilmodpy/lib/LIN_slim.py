...
#import LIN_slim as LIN
#LIZ = LIN.LIN()
#LIZ.Load(32217)
#Kanal 16:
#plt.plot(LIZ.time,LIZ.ne[:,16])
#profile zur zeit 3.2
#plt.plot(LIZ.getRhop(3.2),LIZ(3.2))
...
class LIN:
	...
	def Load( self ,   Shotnumber, Experiment = 'AUGD',Diagnostic = 'LIN', Edition = 0L, tBegin=-1.0, tEnd=12.0 ):	
		self.Unload()
		if Diagnostic == 'LIN':
			try:
			...
			del output
			self.R = numpy.squeeze(sf( 'R' ).data)
		...
		if self.Status:
			...
			if kind == '1D':
				...
				for i in numpy.arange(ntimes):					
					if x[i,0] < x[i,-1]:
						data[i] = numpy.interp(data_in,x[i,:],y[i,:])
					else:
						data[i] = numpy.interp(data_in,x[i,::-1],y[i,::-1])
...
### 2D interpolation version
#			if (data_in == 'ne') & (ne_in != None) :
...
#				#ntimes = numpy.size(time)			
	#			if data_out == 'R':
		...
		#			z = self.zall 
#				IPython.embed()
#				f =  scipy.interpolate.interp2d(time, self.ne, z, kind='linear', fill_value = float('nan'), bounds_error=False)
#				return f(time_in,ne_in)
