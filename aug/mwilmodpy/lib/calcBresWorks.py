...
class calcBres:
	...
	def __init__( self, equShot, equTime=3.0, equExp='AUGD',equDiag='EQI',n=2, nTorAngle=128, nThetaStar = 64,nRhop=1000 ,doPlot=False):
	...
	def loadEqu( self , doPlot=False ):
		...
		if self.statusInit:
			...
			tS = thetaStarClass.thetaStar(shot=self.equShot,time=self.equTime,Diagnostic = self.equDiag,Experiment = self.equExp)
			...
			print 'get q profile'		
			outq = kk.KK().kkrhopfq( self.equShot, self.equTime, tS.rhop, diag=  self.equDiag, exp =  self.equExp )
			qSurf = outq.q
			#embed()
 ...
 ##to truncate the LCFS
	...
	def prepareRotation(self, qRange=None, highDef=False, path_field='/afs/ipp/www/aug/CWAC_Twiki/CoilsData/'):
		if self.statusEqu:
			...
			print 'get B'
			field = kk.KK().kkrzBrzt( self.equShot, self.equTime, surf_r_sym.ravel(), surf_z_sym.ravel() , diag=  self.equDiag, exp =  self.equExp)
...
## read lower coil
			...
			BnCoilSingleU = coilU.BnSingle(Rin=surf_r_sym, zin=surf_z_sym, phiIn = np.deg2rad(torAngle), vecIn=np.array([vec_R_sym,vec_z_sym]))

			BnCoilSingleL = coilL.BnSingle(Rin=surf_r_sym, zin=surf_z_sym, phiIn = np.deg2rad(torAngle), vecIn=np.array([vec_R_sym,vec_z_sym]))
	...
	def rotate(self, phaseShot = None, phaseTime=None, qRange=None,ndiffPhi=50,differential=True,forVideo=False):
		if self.statusEqu:
			...
			if (self.n ==2) & noInput & differential:
				phaseShot = 30680
				phaseTime=np.array([2.233,2.735])
				ndiffPhi=50
				MAWsampling=1.
				qRange = self.qRat
...
#plt.plot(PhaseMPRun,Bres[:])
	...
	def save(self,show=True,rule_of_thumb=True, dt=0.06):		
		if self.statusBres:
			...
			folder = '%d_'%self.equShot+'%.2f_'%self.equTime+self.equExp+'_'+self.equDiag+'_'+'n=%d_'%int(self.n)+self.kind
...
#			plt.axvline(deltaphi_opt_vac)
        ...
        def __call__( self , qIn=None,  BrTime = None, BrShot = None,nTorAngle=128, doPlot=False , doOutput=True, getSpectra=False, getBresProfile = False,usePSL=False):
		...
		if ((doPlot) | (doOutput)) :
			try:
				...
				plt.bar(m4fft-0.4,np.absolute(fftBn[idxQRat,nIdx,:]).T*2.e3)
				plt.bar(m4fft[mIdx]-0.4,np.absolute(fftBn[idxQRat,nIdx,mIdx])*2.e3,color='r')
				plt.xticks( fontsize=ticksize)
				plt.yticks( fontsize=ticksize)
				plt.tight_layout()
				if doOutput:
...
#\rm{_{UL}}
			...
			for i in arr:
				...
				plt.xlabel(r' $ \rm{toroidal} \ \phi $',fontsize=labelsize) 
				plt.ylabel(r' $ \rm{poloidal} \ \Theta^{\star} $',fontsize=labelsize)
				plt.plot(self.torAngle,-self.torAngle/qIn+65.,'k-',linewidth=3)
				plt.yticks(fontsize=ticksize)
