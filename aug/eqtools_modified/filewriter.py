...
def gfile(obj, tin, nw=None, nh=None, shot=None, name=None, tunit = 'ms', title='EQTOOLS', nbbbs=100):
	...
	###########################################################
	#MODIFIED BY PHILIPP ULBL TO INDICATE 2PI CORRECTION
    ###########################################################
    title = title + '2PIC'
    ###########################################################
	...
	###########################################################
	#MODIFIED BY PHILIPP ULBL TO MATCH STANDARD CONVENTION
    ###########################################################
    header = title+ (8-len(title))*' '+ \
                         time.strftime('%m/%d/%y')+ \
                         '        '+' #{:6d}'.format(shot)+ '  {:4s}'.format(stin) + '{:2s}'.format(tunit) + '        '
    ###########################################################
    header = header + '{:4d}'.format(3)+'{:4d}'.format(nw)+'{:4d}'.format(nh)+'\n'
	...
	###########################################################
    #MODIFIED BY PHILIPP ULBL TO CORRECT 2PI IN PSI
    ###########################################################
    psi0 = psi0 / (2*math.pi)
    psiLCFS = psiLCFS / (2*math.pi)
    ###########################################################
	...
	#MODIFIED BY PHILIPP ULBL TO CORRECT 2PI IN PSI
    ###########################################################
    psiRZ = psiRZ / (2*math.pi)
	###########################################################

    gfiler.write(_fmt(psiRZ.ravel()))  #spline with new rz grid

    ###########################################################
    #MODIFIED BY PHILIPP ULBL TO CORRECT LEN OF Q AREA
    ###########################################################
    pts0 = scipy.linspace(0.,1.,obj.getQProfile().shape[-1]) 
    ########################################################### 
