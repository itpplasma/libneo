program bench_spline3d_many
    ...
    num_points(1) = get_env_int("LIBNEO_BENCH_N1", 48)
    num_points(2) = get_env_int("LIBNEO_BENCH_N2", 32)
    num_points(3) = get_env_int("LIBNEO_BENCH_N3", 32)
    if (any(num_points < 6)) error stop "bench_spline3d_many: LIBNEO_BENCH_N1/N2/N3 too small"

    npts = get_env_int("LIBNEO_BENCH_NPTS", 200000)
    if (npts < 1) error stop "bench_spline3d_many: LIBNEO_BENCH_NPTS < 1"

    niter = get_env_int("LIBNEO_BENCH_NITER", 6)
    if (niter < 1) error stop "bench_spline3d_many: LIBNEO_BENCH_NITER < 1"

    nbuild = get_env_int("LIBNEO_BENCH_NBUILD", 2)
    if (nbuild < 1) error stop "bench_spline3d_many: LIBNEO_BENCH_NBUILD < 1"

    nbuild_resident = get_env_int("LIBNEO_BENCH_NBUILD_RESIDENT", nbuild)
    if (nbuild_resident < 1) error stop "bench_spline3d_many: LIBNEO_BENCH_NBUILD_RESIDENT < 1"

    nbuild_repeat = get_env_int("LIBNEO_BENCH_NBUILD_REPEAT", 3)
    if (nbuild_repeat < 1) error stop "bench_spline3d_many: LIBNEO_BENCH_NBUILD_REPEAT < 1"

    neval_repeat = get_env_int("LIBNEO_BENCH_NEVAL_REPEAT", 5)
    if (neval_repeat < 1) error stop "bench_spline3d_many: LIBNEO_BENCH_NEVAL_REPEAT < 1"
    ...
    call bench_cpu()
    npts_old = min(npts, get_env_int("LIBNEO_BENCH_OLD_NPTS", 50000))
...
contains
    ...
    subroutine bench_old_public(npts_local)
        ...
        do it = 1, niter_old
            ...
            do ipt_local = 1, npts_local
                call evaluate_batch_splines_3d(spl, x_eval(:, ipt_local), y_point)
    ...
    subroutine bench_public()
        ...
        t0 = wall_time()
        call evaluate_batch_splines_3d_many(spl, x_eval, y2d)
        t1 = wall_time()
        warmup = t1 - t0
