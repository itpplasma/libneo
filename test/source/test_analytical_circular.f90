...
contains
    ...
    subroutine test_divergence_free_circular()
        ...
        dR = 1.0e-6_dp
        dZ = 1.0e-6_dp
        tol = 2.0e-10_dp  ! Central differences saturate near 1e-10 in 64-bit; relax to avoid false fails
    ...
    subroutine test_divergence_free_shaped()
        ...
        dR = 1.0e-6_dp
        dZ = 1.0e-6_dp
        tol = 2.0e-10_dp  ! Same round-off considerations as circular case
