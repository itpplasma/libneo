...
!
  if ( y <= 0.0D+00 ) then
    ...
    if ( abs(res) > 1.0D-12 ) then  ! Choose a suitable small tolerance

      if ( abs(y1 - aint ( y1 * 0.5D+00 ) * 2.0D+00) > 1.0d-12 ) then
        parity = .true.
      end if
...
!
  ...
  if ( abs(fact - 1.0D+00) > 1.0d-12 ) then
    res = fact / res
  end if
...
!
  ...
  real ( kind = 8 ) a
  real ( kind = 8 ) :: al = 0.0d0
  real ( kind = 8 ) alpha
  real ( kind = 8 ) b
  real ( kind = 8 ) :: be = 0.0d0
  real ( kind = 8 ) beta
  integer ( kind = 4 ) i
  integer ( kind = 4 ) k
  integer ( kind = 4 ) kind
  ...
  real ( kind = 8 ) p
  real ( kind = 8 ) :: shft = 0.0d0
  real ( kind = 8 ) :: slp = 0.0d0
  real ( kind = 8 ) st(nt)
