...
contains
    ...
    recursive subroutine evaluate_batch_splines_1d_der2_core(spl, x, y, dy, d2y)
        ...
        do iq = 1, nq
            y(iq) = spl%coeff(iq, N, idx)
            dy(iq) = real(N, dp)*spl%coeff(iq, N, idx)
            d2y(iq) = real(N*(N - 1), dp)*spl%coeff(iq, N, idx)
        end do

        do k = N - 1, 2, -1
            do iq = 1, nq
                d2y(iq) = real(k*(k - 1), dp)*spl%coeff(iq, k, idx) + &
                          x_local*d2y(iq)
            end do
        end do
        ...
        do k = N - 1, 1, -1
            do iq = 1, nq
                dy(iq) = real(k, dp)*spl%coeff(iq, k, idx) + x_local*dy(iq)
            end do
        end do

        do k = N - 1, 0, -1
            do iq = 1, nq
                y(iq) = spl%coeff(iq, k, idx) + x_local*y(iq)
            end do
        end do
    ...
    recursive subroutine evaluate_batch_splines_1d_der3_core(spl, x, y, dy, d2y, d3y)
        ...
        do iq = 1, nq
            ...
            dy(iq) = real(N, dp)*spl%coeff(iq, N, idx)
            d2y(iq) = real(N*(N - 1), dp)*spl%coeff(iq, N, idx)
            d3y(iq) = real(N*(N - 1)*(N - 2), dp)*spl%coeff(iq, N, idx)
        end do

        do k = N - 1, 3, -1
            do iq = 1, nq
                d3y(iq) = real(k*(k - 1)*(k - 2), dp)*spl%coeff(iq, k, idx) + &
                          x_local*d3y(iq)
            end do
        end do
    ...
    recursive subroutine evaluate_batch_splines_1d_many_der2_resident(spl, x, y_batch, dy_batch, &
        ...
        npts = size(x)
        !$acc parallel loop present(spl%coeff, x, y_batch, dy_batch, d2y_batch)
        do ipt = 1, npts
            call evaluate_batch_splines_1d_der2_core(spl, x(ipt), &
                                                     y_batch(:, ipt), dy_batch(:, &
                                                                ipt), d2y_batch(:, ipt))
        end do
    ...
    recursive subroutine evaluate_batch_splines_1d_many_der2_mask_resident(spl, x, &
        ...
        npts = size(x)
        !$acc parallel loop present(spl%coeff, x, mask, y_batch, dy_batch, d2y_batch)
        do ipt = 1, npts
    ...
    recursive subroutine evaluate_batch_splines_1d_many_der3_resident(spl, x, y_batch, dy_batch, &
        ...
        !$acc& d3y_batch)
        do ipt = 1, npts
            call evaluate_batch_splines_1d_der3_core(spl, x(ipt), &
                                                     y_batch(:, ipt), dy_batch(:, &
                                                              ipt), d2y_batch(:, ipt), &
                                                     d3y_batch(:, ipt))
        end do
    ...
    recursive subroutine evaluate_batch_splines_1d_many_der3_mask_resident(spl, x, &
        ...
        !$acc end parallel loop
    end subroutine evaluate_batch_splines_1d_many_der3_mask_resident
#endif

    recursive subroutine construct_batch_splines_1d(x_min, x_max, y_batch, order, periodic, spl)
        ...
        call construct_batch_splines_1d_legacy(x_min, x_max, y_batch, order, &
                                               periodic, spl)
    ...
    recursive subroutine construct_batch_splines_1d_legacy(x_min, x_max, y_batch, order, periodic, spl)
        real(dp), intent(in) :: x_min, x_max
        real(dp), intent(in) :: y_batch(:, :)  ! (n_points, n_quantities)
        integer, intent(in) :: order
        logical, intent(in) :: periodic
    ...
    recursive subroutine construct_batch_splines_1d_lsq(x_min, x_max, order, periodic, &
    ...
    recursive subroutine allocate_batch_unit(order, num_points, periodic, idx, spl, x_min, x_max)
    ...
    recursive subroutine construct_batch_splines_1d_lines(x_min, x_max, y_batch, order, periodic, spl)
        real(dp), intent(in) :: x_min, x_max
        real(dp), intent(in) :: y_batch(:, :)  ! (n_points, n_quantities)
        integer, intent(in) :: order
        logical, intent(in) :: periodic
        ...
        if (order == 3) then
            if (periodic .and. n_points < 3) then
                error stop &
                    "construct_batch_splines_1d_lines: Need at least 3 points "// &
...
!$omp end parallel do
    ...
    recursive subroutine construct_batch_splines_1d_resident(x_min, x_max, y_batch, order, &
                                                   periodic, spl)
        real(dp), intent(in) :: x_min, x_max
        real(dp), intent(in) :: y_batch(:, :)  ! (n_points, n_quantities)
        integer, intent(in) :: order
    ...
    recursive subroutine construct_batch_splines_1d_resident_device(x_min, x_max, y_batch, &
                                                          order, periodic, spl, &
                                                          update_host, &
                                                          assume_y_present)
        real(dp), intent(in) :: x_min, x_max
    ...
    recursive subroutine construct_batch_splines_1d_resident_device_impl(x_min, x_max, n_points, &
                                                               n_quantities, y_batch, &
                                                               order, periodic, spl, &
                                                               do_update, &
                                                               do_assume_present)
    ...
    recursive subroutine store_coeff_1d(n_points, n_quantities, order, work, coeff)
    ...
    recursive subroutine destroy_batch_splines_1d(spl)
    ...
    recursive subroutine evaluate_batch_splines_1d(spl, x, y_batch)
        type(BatchSplineData1D), intent(in) :: spl
        real(dp), intent(in) :: x
    ...
    recursive subroutine evaluate_batch_splines_1d_single(spl, x, iq, y)
        type(BatchSplineData1D), intent(in) :: spl
        real(dp), intent(in) :: x
    ...
    recursive subroutine evaluate_batch_splines_1d_many(spl, x, y_batch)
        type(BatchSplineData1D), intent(in) :: spl
        real(dp), intent(in) :: x(:)
    ...
    recursive subroutine evaluate_batch_splines_1d_many_resident(spl, x, y_batch)
        type(BatchSplineData1D), intent(in) :: spl
        real(dp), intent(in) :: x(:)
    ...
    recursive subroutine evaluate_batch_splines_1d_many_der(spl, x, y_batch, dy_batch)
        type(BatchSplineData1D), intent(in) :: spl
        real(dp), intent(in) :: x(:)
    ...
    recursive subroutine evaluate_batch_splines_1d_many_der2(spl, x, y_batch, dy_batch, d2y_batch)
        type(BatchSplineData1D), intent(in) :: spl
        real(dp), intent(in) :: x(:)
...
#ifdef _OPENACC
        if (acc_is_present(spl%coeff)) then
            !$acc data copyin(x) copyout(y_batch, dy_batch, d2y_batch)
            call evaluate_batch_splines_1d_many_der2_resident(spl, x, &
                                                              y_batch, &
                                                              dy_batch, d2y_batch)
            !$acc end data
            return
...
#ifdef _OPENACC
        if (acc_is_present(spl%coeff)) then
            !$acc data copyin(x, mask) copy(y_batch, dy_batch, d2y_batch)
            call evaluate_batch_splines_1d_many_der2_mask_resident(spl, x, &
                                                                   mask, y_batch, &
                                                                   dy_batch, &
                                                                   d2y_batch)
            !$acc end data
    ...
    recursive subroutine evaluate_batch_splines_1d_many_der3(spl, x, y_batch, dy_batch, d2y_batch, &
                                                   d3y_batch)
        type(BatchSplineData1D), intent(in) :: spl
        real(dp), intent(in) :: x(:)
        real(dp), intent(out) :: y_batch(:, :), dy_batch(:, :), d2y_batch(:, :), &
...
#ifdef _OPENACC
        if (acc_is_present(spl%coeff)) then
            !$acc data copyin(x) copyout(y_batch, dy_batch, d2y_batch, d3y_batch)
            call evaluate_batch_splines_1d_many_der3_resident(spl, x, y_batch, &
                                                              dy_batch, &
                                                              d2y_batch, &
                                                              d3y_batch)
            !$acc end data
        ...
        do ipt = 1, npts
            call evaluate_batch_splines_1d_der3_core(spl, x(ipt), y_batch(:, ipt), &
                                                     dy_batch(:, ipt), d2y_batch(:, &
                                                                ipt), d3y_batch(:, ipt))
        end do
    ...
    recursive subroutine evaluate_batch_splines_1d_many_der3_mask(spl, x, mask, y_batch, &
        ...
        npts = size(x)
        if (size(mask) /= npts) then
            error stop "evaluate_batch_splines_1d_many_der3_mask: mask size mismatch"
        end if
#ifdef _OPENACC
        if (acc_is_present(spl%coeff)) then
            !$acc data copyin(x, mask) copy(y_batch, dy_batch, d2y_batch, d3y_batch)
            call evaluate_batch_splines_1d_many_der3_mask_resident(spl, x, mask, &
                                                                   y_batch, dy_batch, &
                                                                   d2y_batch, &
                                                                   d3y_batch)
            !$acc end data
    ...
    end subroutine evaluate_batch_splines_1d_many_der3_mask

    recursive subroutine evaluate_batch_splines_1d_der(spl, x, y_batch, dy_batch)
        type(BatchSplineData1D), intent(in) :: spl
        real(dp), intent(in) :: x
        real(dp), intent(out) :: y_batch(:)   ! (n_quantities)
    ...
    recursive subroutine evaluate_batch_splines_1d_der3(spl, x, y_batch, dy_batch, d2y_batch, &
       ...
       !$acc routine seq
        ...
        x_arr(1) = x
        call evaluate_batch_splines_1d_many_der3(spl, x_arr, y_arr, dy_arr, &
                                                 d2y_arr, d3y_arr)
        y_batch(1:spl%num_quantities) = y_arr(:, 1)
        dy_batch(1:spl%num_quantities) = dy_arr(:, 1)
        d2y_batch(1:spl%num_quantities) = d2y_arr(:, 1)
   ...
   end subroutine evaluate_batch_spline_1d_scalar_quintic_der

   subroutine evaluate_batch_spline_1d_pair_quintic_der(spl, x, y1, dy1, y2, dy2)
      !$acc routine seq
      type(BatchSplineData1D), intent(in) :: spl
      real(dp), intent(in) :: x
      real(dp), intent(out) :: y1, dy1, y2, dy2
      ...
      y1 = spl%coeff(1, 5, idx)
      dy1 = 5.0_dp*spl%coeff(1, 5, idx)
      y2 = spl%coeff(2, 5, idx)
      dy2 = 5.0_dp*spl%coeff(2, 5, idx)
      do k = 4, 1, -1
         dy1 = real(k, dp)*spl%coeff(1, k, idx) + x_local*dy1
         dy2 = real(k, dp)*spl%coeff(2, k, idx) + x_local*dy2
      end do
   ...
   end subroutine evaluate_batch_spline_1d_pair_quintic_der
