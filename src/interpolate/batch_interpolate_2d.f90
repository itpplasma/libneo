module batch_interpolate_2d
    ...
    use batch_interpolate_types, only: BatchSplineData2D, MAX_BATCH_ORDER, &
                                       MAX_BATCH_QUANTITIES
...
contains
    ...
    subroutine evaluate_batch_splines_2d_der_core(spl, x, y, dy)
        type(BatchSplineData2D), intent(in) :: spl
        real(dp), intent(in) :: x(2)
        real(dp), intent(out) :: y(:)
        real(dp), intent(out) :: dy(:, :)

        integer :: idx(2), j, iq, k1, k2, nq, N1, N2
        real(dp) :: xj, x_norm(2), x_local(2), period(2)
        real(dp) :: coeff_2(MAX_BATCH_QUANTITIES, 0:MAX_BATCH_ORDER)
        real(dp) :: coeff_2_dx1(MAX_BATCH_QUANTITIES, 0:MAX_BATCH_ORDER)
        ...
        if (nq > MAX_BATCH_QUANTITIES) then
            error stop "evaluate_batch_splines_2d_der_core: too many quantities"
        end if
        if (N1 > MAX_BATCH_ORDER .or. N2 > MAX_BATCH_ORDER) then
            error stop &
                "evaluate_batch_splines_2d_der_core: order exceeds MAX_BATCH_ORDER"
        end if
        if (size(dy, 1) < 2) then
            error stop "evaluate_batch_splines_2d_der_core: dy dim1 must be at least 2"
        end if
        ...
        do j = 1, 2
            if (spl%periodic(j)) then
                xj = modulo(x(j) - spl%x_min(j), period(j)) + spl%x_min(j)
            else
                xj = x(j)
            end if
        ...
        do iq = 1, nq
            do k2 = 0, N2
                ...
                coeff_2_dx1(iq, k2) = real(N1, dp)*spl%coeff(iq, N1, k2, idx(1), idx(2))
            end do
        end do
        ...
        do k1 = N1 - 1, 1, -1
            do iq = 1, nq
                do k2 = 0, N2
                    coeff_2_dx1(iq, k2) = real(k1, dp)*spl%coeff(iq, k1, k2, idx(1), &
                                                                 idx(2)) + &
        ...
        do iq = 1, nq
            y(iq) = coeff_2(iq, N2)
            dy(1, iq) = coeff_2_dx1(iq, N2)
            dy(2, iq) = real(N2, dp)*coeff_2(iq, N2)
        end do

        do k2 = N2 - 1, 0, -1
            do iq = 1, nq
                y(iq) = coeff_2(iq, k2) + x_local(2)*y(iq)
                dy(1, iq) = coeff_2_dx1(iq, k2) + x_local(2)*dy(1, iq)
            end do
        end do

        do k2 = N2 - 1, 1, -1
            do iq = 1, nq
                dy(2, iq) = real(k2, dp)*coeff_2(iq, k2) + x_local(2)*dy(2, iq)
            end do
        end do
    end subroutine evaluate_batch_splines_2d_der_core
...
#else
        call construct_batch_splines_2d_legacy(x_min, x_max, y_batch, order, &
                                               periodic, spl)
#endif
    end subroutine construct_batch_splines_2d

    subroutine construct_batch_splines_2d_legacy(x_min, x_max, y_batch, order, &
                                                 periodic, spl)
        real(dp), intent(in) :: x_min(:), x_max(:)
        real(dp), intent(in) :: y_batch(:, :, :)  ! (n1, n2, n_quantities)
        integer, intent(in) :: order(:)
    ...
    subroutine construct_batch_splines_2d_lines(x_min, x_max, y_batch, order, &
                                                periodic, spl)
        ...
        if (N1_order == 3) then
            if (periodic(1) .and. n1 < 3) then
                error stop &
                    "construct_batch_splines_2d_lines: Need at least 3 points "// &
...
#ifdef _OPENMP
!$omp parallel do default(none) shared(work2, n1, n2, n_quantities, N2_order, &
!$omp& periodic, spl) private(line)
#endif
...
#ifdef _OPENMP
!$omp parallel do default(none) shared(work1, n1, n2, n_quantities, N1_order, &
!$omp& periodic, spl) private(line)
#endif
...
!$omp end parallel do
    ...
    subroutine evaluate_batch_splines_2d_many_mask(spl, x, mask, y_batch)
        type(BatchSplineData2D), intent(in) :: spl
        real(dp), intent(in) :: x(:, :)
        ...
        if (size(x, 1) /= 2) then
            error stop &
                "evaluate_batch_splines_2d_many_mask: First dimension of x must be 2"
        end if
        ...
        if (size(y_batch, 2) /= size(x, 2)) then
            error stop &
                "evaluate_batch_splines_2d_many_mask: y_batch second dim mismatch"
        end if

#ifdef _OPENACC
        if (acc_is_present(spl%coeff)) then
            !$acc data copyin(x, mask) copy(y_batch)
            call evaluate_batch_splines_2d_many_resident_mask(spl, x, mask, y_batch)
            !$acc end data
            return
        end if
#endif
    ...
    subroutine evaluate_batch_splines_2d_many_resident_mask(spl, x, mask, y_batch)
        type(BatchSplineData2D), intent(in) :: spl
        real(dp), intent(in) :: x(:, :)
        logical, intent(in) :: mask(:)
        real(dp), intent(inout) :: y_batch(:, :)
        ...
        !$acc end parallel loop
    end subroutine evaluate_batch_splines_2d_many_resident_mask
    ...
    subroutine evaluate_batch_splines_2d_many_der(spl, x, y_batch, dy_batch)
        ...
        if (size(x, 1) /= 2) then
            error stop &
                "evaluate_batch_splines_2d_many_der: First dimension of x must be 2"
        end if
        if (size(y_batch, 2) /= size(x, 2)) then
            error stop "evaluate_batch_splines_2d_many_der: y_batch second dim mismatch"
        end if
        if (size(dy_batch, 3) /= size(x, 2)) then
            error stop "evaluate_batch_splines_2d_many_der: dy_batch dim mismatch"
        end if
...
#ifdef _OPENACC
        if (acc_is_present(spl%coeff)) then
            !$acc data copyin(x) copyout(y_batch, dy_batch)
            call evaluate_batch_splines_2d_many_der_resident(spl, x, y_batch, dy_batch)
            !$acc end data
            return
        end if
#endif
        ...
        do ipt = 1, npts
            call evaluate_batch_splines_2d_der_core(spl, x(:, ipt), y_batch(:, ipt), &
                                                    dy_batch(:, :, ipt))
        end do
    end subroutine evaluate_batch_splines_2d_many_der

    subroutine evaluate_batch_splines_2d_many_der_resident(spl, x, y_batch, dy_batch)
        type(BatchSplineData2D), intent(in) :: spl
        real(dp), intent(in) :: x(:, :)              ! (2, npts)
        real(dp), intent(inout) :: y_batch(:, :)     ! (nq, npts)
        real(dp), intent(inout) :: dy_batch(:, :, :)  ! (2, nq, npts)

        integer :: ipt, npts

        npts = size(x, 2)
        !$acc parallel loop present(spl%coeff, x, y_batch, dy_batch)
        do ipt = 1, npts
            call evaluate_batch_splines_2d_der_core(spl, x(:, ipt), y_batch(:, ipt), &
                                                    dy_batch(:, :, ipt))
        end do
        !$acc end parallel loop
    end subroutine evaluate_batch_splines_2d_many_der_resident

    subroutine evaluate_batch_splines_2d_many_der_mask(spl, x, mask, y_batch, dy_batch)
        type(BatchSplineData2D), intent(in) :: spl
        real(dp), intent(in) :: x(:, :)              ! (2, npts)
        logical, intent(in) :: mask(:)
        real(dp), intent(inout) :: y_batch(:, :)     ! (nq, npts)
...
#ifdef _OPENACC
        if (acc_is_present(spl%coeff)) then
            !$acc data copyin(x, mask) copy(y_batch, dy_batch)
            call evaluate_batch_splines_2d_many_der_mask_resident(spl, x, mask, &
                                                                  y_batch, dy_batch)
            !$acc end data
            return
        end if
    ...
    subroutine evaluate_batch_splines_2d_many_der_mask_resident(spl, x, mask, &
                                                                y_batch, dy_batch)
        type(BatchSplineData2D), intent(in) :: spl
        real(dp), intent(in) :: x(:, :)              ! (2, npts)
        logical, intent(in) :: mask(:)
        ...
        npts = size(x, 2)
        !$acc parallel loop present(spl%coeff, x, mask, y_batch, dy_batch)
        do ipt = 1, npts
            if (.not. mask(ipt)) cycle
            call evaluate_batch_splines_2d_der_core(spl, x(:, ipt), y_batch(:, ipt), &
                                                    dy_batch(:, :, ipt))
        ...
        !$acc end parallel loop
    end subroutine evaluate_batch_splines_2d_many_der_mask_resident
