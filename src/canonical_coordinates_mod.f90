...
module new_vmec_stuff_mod
  ...
  real(dp) :: rmajor, h_theta, h_phi
  real(dp) :: vmec_B_scale=1d0, vmec_RZ_scale=1d0
  real(dp), dimension(:), allocatable :: axm, axn, soa
  real(dp), dimension(:), allocatable :: aiota, s, sps, phi
  real(dp), dimension(:,:), allocatable :: almnc, rmnc, zmnc
  real(dp), dimension(:,:), allocatable :: almns, rmns, zmns

  real(dp), dimension(:,:,:,:,:,:), allocatable :: sR, sZ, slam
...
module vector_potentail_mod
  ...
  integer :: ns
  real(dp) :: hs,torflux
  real(dp), dimension(:,:), allocatable :: sA_phi
...
module canonical_coordinates_mod
  ...
  real(dp) :: hs_c, h_theta_c, h_phi_c
  real(dp), dimension(:,:,:), allocatable :: G_c, sqg_c,&
      & B_vartheta_c, B_varphi_c, A_vartheta_c, A_varphi_c, Delta_varphi_c, chi_gauge,&
      & Bmod_c

  real(dp), dimension(ns_max)                     :: derf1, derf2, derf3
  real(dp), dimension(:,:,:,:,:,:),   allocatable :: s_G_c
...
module canonical_coordinates_new_mod
  ...
  integer, parameter :: nh_stencil=3
  real(dp) :: hs_c, h_theta_c, h_phi_c
  real(dp), dimension(:,:,:), allocatable :: Delta_varphi_c, &
      & h_vartheta_c, h_varphi_c, A_vartheta_c, A_varphi_c, chi_gauge, Bmod_c

  real(dp), dimension(ns_max)                     :: derf1, derf2, derf3
  real(dp), dimension(:,:,:,:,:,:),   allocatable :: s_Delta_varphi_c
  real(dp), dimension(:,:,:,:,:,:,:), allocatable :: s_Bmod_B_A
...
module boozer_coordinates_mod
  ...
  real(dp), dimension(:,:,:), allocatable :: s_Bcovar_tp_B

  ! spline coefficients for Boozer $B$ and $B_r$:
  real(dp), dimension(:,:,:,:,:,:), allocatable :: s_Bmod_B, s_Bcovar_r_B
  ...
  real(dp), dimension(:,:,:,:,:,:,:), allocatable :: s_delt_delp_V, s_delt_delp_B
