module neo_field
    ...
    subroutine create_field(field, field_type, ampl, ampl2, &
                                               coils_file, &
                                               limits, field_to_interpolate, n_nodes)
        ...
        select case(field_type)
            ...
            case("polylag")
                allocate(field, source= & 
                            create_polylag_field(limits, field_to_interpolate, n_nodes))
            case default
    ...
    function create_polylag_field(limits, field, n_nodes) result(polylag_field)
