...
!$omp end parallel do
   ...
   subroutine biot_savart_fourier(coils, nmax, &
      ...
      call neo_fft_plan_init(plan_nphi, nphi)
      do kc = 1, ncoil
         do kZ = 1, nZ
            do kR = 1, nR
...
!$omp end parallel do
   ...
   subroutine vector_potential_biot_savart_fourier(coils, nmax, min_distance, max_eccentricity, use_convex_wall, &
      ...
      integer :: ncoil, kc, ks, ks_prev, kR, kphi, kZ
      real(dp), dimension(nphi) :: phi, cosphi, sinphi
...
!$omp end parallel do
   ...
   subroutine sum_coils_gauge_single_mode_Anvac(AnR, Anphi, AnZ, dAnphi_dR, dAnphi_dZ, &
      ...
      complex(dp), dimension(0:, :, :, :), intent(in) :: AnR, Anphi, AnZ, dAnphi_dR, dAnphi_dZ
      ...
      ncoil = size(Ic)
      if (lbound(AnR, 1) /= 0) then
         write (error_unit, '("sum_coils_gauge_single_mode_Anvac: AnR lbound(1) must be 0")')
         error stop
      end if
      if (size(AnR, 2) /= nR .or. size(AnR, 3) /= nZ .or. size(AnR, 4) /= ncoil) then
         write (error_unit, '("sum_coils_gauge_single_mode_Anvac: AnR shape mismatch")')
         error stop
      end if
      if (size(Anphi, 2) /= nR .or. size(Anphi, 3) /= nZ .or. size(Anphi, 4) /= ncoil) then
         write (error_unit, '("sum_coils_gauge_single_mode_Anvac: Anphi shape mismatch")')
         error stop
      end if
      if (size(AnZ, 2) /= nR .or. size(AnZ, 3) /= nZ .or. size(AnZ, 4) /= ncoil) then
         write (error_unit, '("sum_coils_gauge_single_mode_Anvac: AnZ shape mismatch")')
         error stop
      end if
      if (ubound(AnR, 1) < max(0, ntor)) then
         write (error_unit, '("sum_coils_gauge_single_mode_Anvac: AnR ubound(1) < ntor")')
         error stop
      end if
   ...
   subroutine write_Anvac_fourier(filename, ncoil, nmax, &
      ...
      real(dp), intent(in) :: Rmin, Rmax, Zmin, Zmax
      integer, intent(in) :: nR, nphi, nZ
      complex(dp), intent(in), dimension(0:, :, :, :) :: AnR, Anphi, AnZ, dAnphi_dR, dAnphi_dZ
      real(dp) :: R(nR), Z(nZ)
