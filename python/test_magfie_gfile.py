...
def orbit(R0 = 10., phi0 = 0., Z0 = -50., vpar0 = 0.9, v = 1.38*v0, plotting=False):
    ...
    libmc_efit.field(R0 + raxis, phi0, Z0, Br, Bp, Bz, dBrdR, dBrdp, dBrdZ,
                         dBpdR, dBpdp, dBpdZ, dBzdR, dBzdp, dBzdZ)
    psi_pol = field_eq.psif - field_eq.psib
    Aphi = -psi_pol/(R0 + raxis)
    ...
    plt.plot(zs[0,0]-raxis, zs[2,0], 'gx')
    plt.plot(zs[0,-1]-raxis, zs[2,-1], 'r+')
#    plt.title('Poloidal Projection; r = '+str(R0)+', z0 = '+str(Z0)+', vpar/v = '+ str(vpar0))
    plt.title('Poloidal Projection (cylindrical coord. field)')
    plt.axis('equal')
    plt.xlabel('R - $R_0$ / cm')
    plt.ylabel('Z / cm')
    plt.show()
...
plotting = False
plt.show()
v = np.sqrt(10000*ev*2/(am*p_mass))/v0
...
#    v = ((vb - va)*random() + va)*v0
    ...
    solution[k,:] = orbit(R0=r0, Z0=z0, vpar0=vpar_v, v=v, plotting=True)
    plotting = False
