...
def main() -> None:
    ...
    test_grid, AnR, Anphi, AnZ, dAnphi_dR, dAnphi_dZ = read_Anvac_fourier(str(test_path))

    ref_spl = spline_gauged_Anvac(ref_grid, *gauged_Anvac_from_Bnvac(ref_grid, BnR_ref, Bnphi_ref, BnZ_ref))
    test_spl = spline_gauged_Anvac(test_grid, *gauge_Anvac(test_grid, AnR, Anphi, AnZ, dAnphi_dR, dAnphi_dZ))

    if not np.allclose(ref_grid.R, test_grid.R) or not np.allclose(ref_grid.Z, test_grid.Z):
        raise ValueError("Reference and test grids differ; please resample before plotting")
    ...
    for idx, spl in enumerate((ref_spl, test_spl)):
        ...
        magnitude_sq = (BnR * np.conj(BnR) + Bnphi * np.conj(Bnphi) + BnZ * np.conj(BnZ)).real
