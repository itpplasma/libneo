...
def _parse_coils(path: str) -> Coils:
    ...
    - Recognizes: 'periods', 'begin filament', optional 'end filament', 'mirror'.
    ...
    with open(path, "r", encoding="utf-8") as f:
        for lineno, raw in enumerate(f, start=1):
            ...
            if low.startswith("periods"):
                # Record periods if parseable (also parsed in a quick pass below)
                toks = line.split()
                if len(toks) > 1:
                    try:
                        periods = int(toks[1])
            ...
            if low.startswith("begin filament"):
                # Close any in-progress filament, then start a new one
                if in_filament and len(xs) > 0:
            ...
            if in_filament:
                ...
                if len(parts) < 4:
                    raise ValueError(
                        f"Expected 4 numeric fields (x y z I) at {path}:{lineno}"
                    )
                x = float(parts[0])
                y = float(parts[1])
                z = float(parts[2])
