...
def eqdsk2vmec_gfile(filename):
    ...
    for i in range(len(pflux)):
        torflux[i], _ = quad(
            qprof_interp, pflux[0], pflux[i]
        )  # EQDSK uses flux in Weber/rad
        ...
        for n1 in range(2 * ntor + 1):
            for i in range(nu):
                for j in range(nv):
                    data2["refou"][m1, n1] += (
                        R[i]
                        * (cosv[j, n1] * cosu[i, m1] - sinv[j, n1] * sinu[i, m1])
                        * fnuv[m1]
                    )
                    data2["zefou"][m1, n1] += (
                        Z[i]
                        * (sinv[j, n1] * cosu[i, m1] + cosv[j, n1] * sinu[i, m1])
                        * fnuv[m1]
                    )
                    data2["refou2"][m1, n1] += (
                        R[i]
...
def write_vmec_input(filename, data):
    ...
    try:
        with open(filename, "w") as f:
            ...
            f.write(
                "  NS_ARRAY = "
                + " ".join(f"{val:8d}" for val in data["ns_array"])
                + "\n"
            )
            ...
            if "niter_array" in data:
                f.write(
                    "  NITER_ARRAY = "
                    + " ".join(f"{val:8d}" for val in data["niter_array"])
                    + "\n"
                )

            f.write(
                "  FTOL_ARRAY = "
                + " ".join(f"{val:.2E}" for val in data["ftol_array"])
                + "\n"
            )
            ...
            for i in range(data["mpol"]):
                for j in range(2 * data["ntor"] + 1):
                    if data["rbc"][j, i] != 0.0 or data["zbs"][j, i] != 0.0:
                        f.write(
                            f'  RBC({j-data["ntor"]},{i}) = {data["rbc"][j,i]:.10e}  ZBS({j-data["ntor"]},{i}) = {data["zbs"][j,i]:.10e}\n'
                        )
                        if data["lasym"]:
                            f.write(
                                f'     RBS({j-data["ntor"]},{i}) = {data["rbs"][j,i]:.10e}  ZBC({j-data["ntor"]},{i}) = {data["zbc"][j,i]:.10e}\n'
                            )

            f.write(
                "!----- Created by write_vmec "
...
def read_vmec_out_data(filename):
    ...
    data["dimensions"] = {
        dim: dataset.dimensions[dim].size for dim in dataset.dimensions
    }
    ...
    for var_name in dataset.variables:
        ...
        variables[var_name] = {"data": var_data, "attributes": var_attrs}
...
def vmec_to_eqdsk(vmec_data):
    ...
    iota_interp = interp1d(
        phi,
        vmec_data["variables"]["iotas"]["data"],
        kind="linear",
    ...
    xgrid, zgrid = np.meshgrid(np.linspace(r1, r2, nx), np.linspace(z1, z2, nz))
    psixz = Fchi(xgrid, zgrid)
    psixz = np.where(np.isnan(psixz), 0.0, psixz)

    eqdata = {}
    eqdata["header"] = (
        f"{'VMEC ':<10}{datetime.now().strftime('%m/%d/%Y'):>10}{'         # xxxxx':>10}{'  0000ms':>10}           0  "
    )
    ...
    if "q_factor" in vmec_data["variables"]:
        ...
        eqdata["qprof"] = PchipInterpolator(np.linspace(0, 1, len(q_factor)), q_factor)(
            np.linspace(0, 1, nx)
        )
    else:
